...
const myLoader = ({ src, width }: any) => {
  return `https://icongr.am/fontawesome/${src}?${width}`;
};

let nivel = 4;

export default function Home() {
  const [images, setImages] = useState(getImages(nivel));
  ...
  const handleClick = (images: string) => {
    if (empezar) {
      if (selected.length < 2) {
        setSelected((selected) => selected.concat(images));
      }
    }
  };
            ...
            <>
              <div className={home.time}>
                <div>
                  {time >= 59999 ? (
                    <span>
                      {("0" + Math.floor((time / 60000) % 100)).slice(-2)}:
                    </span>
                  ) : (
                    ""
                  )}
              ...
              <div className={home.time__btns}>
                {!empezar && !timerOn && time === 0 ? (
                  <button
              ...
              <div className={home.content__principal}>
                <div className={home.nivel}>
                  <Image
                    src={"/assets/estrellita.png"}
                    width={40}
                    height={30}
                    alt="nivel_1"
                    className={
                      nivel >= 7 ? home.nivel__nostar_1 : home.nivel__nostar
                    }
                  />
                  <Image
                    src={"/assets/estrellita.png"}
                    ...
                    alt="nivel_2"
                    className={
                      nivel >= 10 ? home.nivel__nostar_1 : home.nivel__nostar
                    }
                  />
                  <Image
                    src={"/assets/estrellita.png"}
                    ...
                    alt="nivel_3"
                    className={
                      nivel >= 13 ? home.nivel__nostar_1 : home.nivel__nostar
                    }
                  />
                </div>
                <ul className={nivel >= 7 ? home.content__2 : home.content}>
                  {images.map((images) => {
                    const [, url] = images.split("|");
                    return (
                      <li
                        key={images}
                        className={home.list_img}
                        onClick={() => handleClick(images)}
                      >
                        {selected.includes(images) ||
                        guessed.includes(images) ? (
                          <Image
                            src={url}
                            alt="catito"
                            width={120}
                          ...
                          <Image
                            loader={myLoader}
                            priority
                            src={"pagelines.svg?size=63&color=6a105e"}
                            alt="catito"
                            width={120}
                            height={120}
                            className={home.front}
                          />
                        )}
                      </li>
                    );
                  ...
                  <div className={home.link}>
                     
                    <a
                      href="https://github.com/Macarena-Barboza"
                      target="_blank "
                    >
                      <svg
                        className={home.link__ico}
